...
| **B. Client–Worker 连接长期均衡** | 除**一次性故障**场景外，希望各 Worker 上 **C↔W 连接数/数据面压力** 在**稳态**下**不过分偏斜**（在现有连接模型下可度量的范围内）。 | 与 A 使用同一套 **Power-of-Two**：先以 **连接数** 为 Po2 比较信号验证；再以 **近窗 UB 字节**（业务方/类鲲鹏口径，见 [README.md](./README.md) §1）为生产级信号。 |
...
| **B** | **近窗 UB 累计字节**（C↔W + W↔W，[README.md](./README.md) §1 结论） | **业务方/类鲲鹏** 数据面瓶颈下的生产口径；在 A 稳定后替换比较量。 |
