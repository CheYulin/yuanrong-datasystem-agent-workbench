# RFC：以 UB 数据面吞吐刻画 Worker 负载（类鲲鹏超节点 / 业务方场景）
...
- **环境**：在 **类鲲鹏超节点** 上，**UB（Unified Buffer / 统一缓冲相关数据面，经 URMA 等路径）的带宽** 通常是制约 Worker **存储与读取** 的最关键资源，而不是单纯的 CPU 或单段时延。
...
- **场景陈述**：在 **业务方类** 部署与 access pattern 下，上述并集被视为 **对 Worker 压力最贴近的单一负载指标**（优于仅用 RPC 次数、或仅用某一类时延分桶）。
...
| **里程碑 B（生产/业务方口径）** | **Po2 + UB 传输字节** | 在 A 跑通后，将 Po2 的比较量 **换为** 上表 §1 的 **近窗 UB 字节**（C↔W + W↔W）。仍 **不** 要求在本里程碑同时上 jitter/熔断/重试。 |
