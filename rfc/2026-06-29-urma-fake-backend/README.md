...
| Source HEAD | `95009cc5bfe15b5432d282091e22ade97a9c7f82` |
...
| PR 内设计记录 | `docs/source_zh_cn/design_document/urma_mock_validation_20260628.md` |

## 目标

1. **无 RNIC 环境验证 URMA 语义**：用共享内存和 UDS 模拟本节点内的远端 DMA 行为，使 Object/KV URMA ST/UT 能在无真实 URMA 设备环境运行。
2. **业务路径尽量无感**：mock 行为收敛在 dlopen/ABI/mock backend 边界，业务逻辑继续以 `USE_URMA` 为主，避免把 mock 特化扩散到 Object/KV 代码。
...
4. **保留真实 URMA 约束**：mock 只验证语义正确性，不宣称性能等价；2 GiB、completion byte count、JFC/Jetty 生命周期等约束按真实路径解释。
