...
- Profile: `daily.full`.
- Thresholds: coverage and perf regression thresholds from `.skills/ds-harness/references/profiles.yaml`.

## Commands

```bash
python3 .skills/ds-harness/scripts/ds_harness.py daily --profile daily.full
python3 .skills/ds-harness/scripts/ds_harness.py daily --profile daily.full --dry-run --json
```

The profile owns the full smoke/UT/ST sweep, coverage collection via `yuanrong-datasystem/build.sh -c`, and
performance regression via `.skills/ds-daily/scripts/perf/zmq_rpc_perf_nightly.sh`.
