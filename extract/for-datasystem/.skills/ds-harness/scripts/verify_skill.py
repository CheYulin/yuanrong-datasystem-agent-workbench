...
def run_cmd(cmd: list[str], *, cwd: Path = REPO_ROOT, env: dict[str, str] | None = None) -> tuple[int, str]:
    proc = subprocess.run(
        cmd,
        cwd=cwd,
        text=True,
...
def verify_skill(
    ...
    *,
    ...
    summary = {
        ...
        "evidence_dir": str(out_dir.relative_to(REPO_ROOT)),
...
def sync_for_node(node: str) -> None:
    if node == "xqyun-32c32g":
        script = REPO_ROOT / "scripts/development/sync/sync_to_xqyun.sh"
    else:
        script = REPO_ROOT / "scripts/harness/sync_workspace_to_tiantiyun.sh"
...
def node_ssh_target(node: str) -> tuple[str, str]:
    with (REPO_ROOT / "scripts/config/nodes.yaml").open(encoding="utf-8") as f:
