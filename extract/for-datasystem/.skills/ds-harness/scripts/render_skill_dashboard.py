#!/usr/bin/env python3
"""Render workbench skill verification dashboard from results/ds-skill-runs/manifest.json."""
...
RUNS = REPO_ROOT / "results" / "ds-skill-runs"
...
def load_skill_summary(evidence_dir: str) -> dict:
    path = REPO_ROOT / evidence_dir / "summary.json"
    if path.is_file():
        return json.loads(path.read_text(encoding="utf-8"))
    return {}
...
<tbody>
{''.join(rows) if rows else '<tr><td colspan="5">无 ds-skill-runs 数据 — 先运行 verify_skill.sh</td></tr>'}
</tbody>
</table>
</body>
</html>
...
def main() -> int:
    ...
    out = Path(args.output) if args.output else REPO_ROOT / "results" / f"skill_dashboard_{stamp}.html"
