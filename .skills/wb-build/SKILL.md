...
- Node: default `tiantiyun-80c128g` from `scripts/config/nodes.yaml`.
- Profile: `build.quick` or `build.full` from `scripts/harness/profiles.yaml`.

## Commands

```bash
python3 scripts/harness/ds_harness.py build --backend cmake --profile build.quick
python3 scripts/harness/ds_harness.py build --backend bazel --profile build.full
python3 scripts/harness/ds_harness.py build --backend cmake --dry-run --json
```

Compatibility wrappers may still call `scripts/build/build_cmake.sh`, `scripts/build/build_bazel.sh`, or
`scripts/build/rsync_datasystem_remote_bazel.sh`, but the owner is `wb-build`.

## Evidence

Harness runs write `results/harness/<timestamp>-<profile>/` with:
